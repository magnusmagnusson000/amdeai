#!/usr/bin/env bash
# Ensure Qwen appears in AI Workbench Chat model selector.
#
# Workbench /api/namespaces/<ns>/chattable requires the AIMClusterModel to expose a
# "chat" tag in status.imageMetadata. With spec.discovery.extractMetadata=false the
# AIM operator skips copying spec.imageMetadata into status — remove that flag so the
# chat tag is published.
#
# Usage:
...
echo "=== ensure-qwen-chattable (${MODEL}) ==="

if ! kubectl get aimclustermodel "$MODEL" &>/dev/null; then
  echo "ERROR: AIMClusterModel ${MODEL} not found — run CATALOG_ONLY=1 scripts/10-qwen3-6-27b.sh"
  exit 1
fi
