#!/usr/bin/env bash
# Ensure DiffusionGemma appears in AI Workbench Chat model selector.
#
# Usage:
...
echo "=== ensure-diffusiongemma-chattable (${MODEL}) ==="

if ! kubectl get aimclustermodel "$MODEL" &>/dev/null; then
  echo "ERROR: AIMClusterModel ${MODEL} not found — run CATALOG_ONLY=1 scripts/12-diffusiongemma-26b.sh"
  exit 1
fi
