...
SLM="${SLM_MODEL:-/home/magnus/projects/oracle/models/Qwen3.5-0.8B-Q4_K_M.gguf}"
...
"$BUILD/bin/llama-cli" -m "$SLM" -ngl 99 -c 2048 -n 16 \
  -p "What is 2+2? Answer briefly." --no-display-prompt --simple-io -fa 0

if [[ -f "$GEMMA" ]]; then
  echo "=== Gemma 4 MoE test (HIP): $GEMMA ==="
  OUT=$("$BUILD/bin/llama-cli" -m "$GEMMA" -ngl 99 -c 4096 -n 32 \
    -p "Hello, introduce yourself in one sentence." --no-display-prompt --simple-io -fa 0 2>&1) || true
  echo "$OUT"
